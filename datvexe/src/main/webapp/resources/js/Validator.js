...
Validator.isNumberPhoneLength10AndStartWith0123 = function(selector, message) {
	return {
		selector: selector,
		test: function(value) {
			let check = value.startsWith("0123") && value.length == 10;
			return check ? undefined : message || 'Số điện thoại phải là số có 10 chữ số và bắt đầu bằng “0123”';
		}
	};
}
...
Validator.isGreaterThanOrEqualCurrentDate = function(selector, message) {
	return {
		...
		test: function(value) {
			...
			let specifiedDate = moment(value);
			return today.isSameOrBefore(specifiedDate) ? undefined : message || 'Lớn hơn hoặc bằng ngày hiện tại';
		}
	};
}
...
Validator.isCondition = function(selector, callback, message) {
	return {
		selector: selector,
		test: function() {
			return callback() ? undefined : message || 'Không hợp lệ';
		}
	};
}
